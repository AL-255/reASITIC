...
def write_spice_subckt(
    ...
    *,
    ...
    # SPICE topology is a parallel-RC port shunt to ground, so we
    # use the parallel-RC values from PiResult (Im(Y_p)/ω and
    # 1/Re(Y_p)), not the series-RC ones the C textual output prints.
    out.write(f"Cp1 p1 gnd  {pi.Cp_parallel_p1_fF:.6g}f\n")
    out.write(f"Cp2 p2 gnd  {pi.Cp_parallel_p2_fF:.6g}f\n")
    if pi.g_p1 > 0:
