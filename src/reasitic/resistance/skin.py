...
def ac_resistance_segment(
    *,
    ...
    if xi >= 2.5:
        # High-freq branch — direct port of the C ``ξ ≥ 2.5`` arm of
        # ``compute_inductance_inner_kernel`` (asitic_kernel.c:0x0804d1e4):
        #   ratio = 0.0035 · (W/T − 1)^1.8
        #         + (1.1147 + 1.2868·ξ) / (1.2296 + 1.287·ξ³)
        #         + (ξ · 0.43093) / (1 + 0.041 · (W/T)^1.19)
        # The earlier Python port had the (W/T)^1.19 and (W/T-1)^1.8
        # terms swapped, producing ratio < 1 (i.e. negative skin
        # effect) in mid-frequency ranges. The C reads the W/T value
        # from segment width / metal-stack thickness (the table entry
        # at offset 0xb0) and only computes the (W/T-1)^1.8 term when
        # the aspect ratio is > 1 (else lVar3 stays at 0).
        aspect_minus_1 = max(0.0, aspect - 1.0)
        u = aspect_minus_1 ** 1.8     # (W/T - 1)^1.8
        v = aspect ** 1.19            # (W/T)^1.19
        ratio = (
            0.0035 * u
...
def compute_ac_resistance(shape: Shape, tech: Tech, freq_ghz: float) -> float:
    ...
    metal layer's rsh and thickness from ``tech``. Via filaments
    (``segment.metal >= len(tech.metals)``) contribute their
    per-cluster DC R (parallel combination of all cells), matching
    the early-return for vias in
    ``compute_inductance_inner_kernel`` (asitic_kernel.c:0x0804d1e4)
    which returns ``R_per_via / (cells_x * cells_y)`` *regardless* of
    frequency — vias have no AC enhancement.
    """
