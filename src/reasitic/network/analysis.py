...
class PiResult:
    ...
    commands print: an inductance and series resistance for the
    through path, plus a series-RC equivalent for each shunt branch.

    Per the C's ``extract_pi_equivalent`` (``asitic_kernel.c:0x08089e40``)
    each shunt is decomposed in the **impedance domain**::

        Z_{p,k} = 1 / Y_{p,k} = R_{s,k} + 1 / (j \\omega C_{s,k})

    so ``R_{s,k} = Re(Z_{p,k})`` and ``C_{s,k} = -1 / (\\omega · Im(Z_{p,k}))``.
    This is the canonical series-RC port model the binary's ``Pi2``
    command prints; the alternative parallel-RC interpretation
    ``Y_{p} = G + j\\omega C`` does not match the C's textual output
    (in particular it gives R values 5-50× too high).

    Attributes:
        freq_ghz: Operating frequency, GHz.
        L_nH:     Through-path inductance, nH.
        R_series: Through-path resistance, Ω.
        C_p1_fF:  Port-1 shunt series cap (canonical C output), fF.
        C_p2_fF:  Port-2 shunt series cap (canonical C output), fF.
        Rs_p1:    Port-1 shunt series resistance, Ω.
        Rs_p2:    Port-2 shunt series resistance, Ω.
        Cp_parallel_p1_fF: Parallel-RC equivalent ``Im(Y_p1)/ω``, fF.
        Cp_parallel_p2_fF: Same for port 2.
        g_p1:     Port-1 shunt conductance ``Re(Y_p1)``, S — kept
                  for callers (SPICE export, Pi3, Pi4) that need
                  the parallel-RC interpretation.
        g_p2:     Same for port 2.
    """
    ...
    Rs_p1: float
    Rs_p2: float
    Cp_parallel_p1_fF: float
    Cp_parallel_p2_fF: float
    g_p1: float  # parallel-RC equivalent: Re(Y_p1)
    g_p2: float  # parallel-RC equivalent: Re(Y_p2)


def pi_model_at_freq(shape: Shape, tech: Tech, freq_ghz: float) -> PiResult:
    """Build the Pi-equivalent of ``shape`` at ``freq_ghz`` and break
    out the Z_s / Y_p values into physical L, R, C.

    Mirrors the binary's ``cmd_pi_emit`` and ``extract_pi_equivalent``
    (``asitic_repl.c`` / ``asitic_kernel.c:0x08089e40``).
    """
    if freq_ghz <= 0:
    ...
    L_nH = L_H / NH_TO_H
    # Parallel-RC values (kept for callers that want G in S)
    g1 = float(pi.Y_p1.real)
    g2 = float(pi.Y_p2.real)
    Cp1_parallel_fF = float(pi.Y_p1.imag) / omega * 1.0e15
    Cp2_parallel_fF = float(pi.Y_p2.imag) / omega * 1.0e15
    # Series-RC decomposition (matches C's textual Pi2 output): take
    # Z_p = 1/Y_p, then Cs = -1/(ω·Im(Z_p)), Rs = Re(Z_p). When
    # |Y_p| is below numerical noise we fall back to "open" values
    # (1e15 Ω cap impedance, 1e15 fF cap; mirroring the C's
    # ``local_7c = 1e+15`` branch when ``|Y_p| ≤ 1e-10``).
    if abs(pi.Y_p1) <= 1.0e-15:
        C1_fF = 0.0
        Rs1 = 1.0e15
    else:
        Zp1 = 1.0 / pi.Y_p1
        Rs1 = float(Zp1.real)
        imZ1 = float(Zp1.imag)
        C1_fF = (-1.0 / (omega * imZ1)) * 1.0e15 if imZ1 < 0 else 0.0
    if abs(pi.Y_p2) <= 1.0e-15:
        C2_fF = 0.0
        Rs2 = 1.0e15
    else:
        Zp2 = 1.0 / pi.Y_p2
        Rs2 = float(Zp2.real)
        imZ2 = float(Zp2.imag)
        C2_fF = (-1.0 / (omega * imZ2)) * 1.0e15 if imZ2 < 0 else 0.0
    return PiResult(
        freq_ghz=freq_ghz,
        L_nH=L_nH,
        R_series=R,
        ...
        Rs_p1=Rs1,
        Rs_p2=Rs2,
        Cp_parallel_p1_fF=Cp1_parallel_fF,
        Cp_parallel_p2_fF=Cp2_parallel_fF,
        g_p1=g1,
        g_p2=g2,
    )
...
def pi3_model(
    ...
    *,
    ...
    return Pi3Result(
        ...
        # Substrate-loss resistance in the C's series-RC port model.
        R_sub_p1_ohm=pi.Rs_p1,
        R_sub_p2_ohm=pi.Rs_p2,
    )
