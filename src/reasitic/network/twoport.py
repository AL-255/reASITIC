...
def spiral_y_at_freq(
    ...
    *,
    ...
    if (y_p1 is None or y_p2 is None) and include_substrate:
        ...
        if use_segment_cap:
            # ``use_segment_cap=True`` switches to the C-faithful FFT
            # Green's-function path (``compute_green_function`` +
            # ``analyze_capacitance_driver`` at asitic_kernel.c:0x0808c350
            # / 0x08052c50). Total cap comes from the shape-level
            # ``substrate_cap_matrix`` (which now uses the spectral
            # kernel ported in :func:`spectral_green_layered` and a
            # PEC ground with ε_eff). Asymmetry between Cs1, Cs2 still
            # comes from the per-segment quadrature in
            # :func:`shape_pi_capacitances`, which captures the
            # spatial distribution of segments along the spiral. The
            # substrate conductance G is taken from
            # ``shape_shunt_admittance`` (TODO §7 lists this as the
            # remaining bit until the spectral kernel is also used
            # for σ-derived G).
            from reasitic.substrate import (
                shape_pi_capacitances,
                shape_shunt_admittance,
            )
            from reasitic.substrate.fft_grid import substrate_cap_matrix

            try:
                C_fft = substrate_cap_matrix(
                    [shape], tech, nx=128, ny=128,
                )[0, 0]
            except Exception:
                C_fft = 0.0
            try:
                C_p1, C_p2, _ = shape_pi_capacitances(
                    shape, tech, n_div=n_div,
                )
                c_sum = C_p1 + C_p2
                if c_sum > 0:
                    r1 = C_p1 / c_sum
                ...
                r1 = r2 = 0.5
            Y_loss = shape_shunt_admittance(shape, tech, freq_ghz)
            Y_total = complex(Y_loss.real, omega * C_fft)
            Y_sub_p1 = Y_total * r1
            Y_sub_p2 = Y_total * r2
        else:
            # Default path: ``shape_shunt_admittance`` returns the
            # lossy substrate admittance Y = G + jωC at this frequency
            # (magnitudes validated against gold within ~5 % for the
            # canonical BiCMOS spirals). Split asymmetrically between
            # the two ports using the per-port row-sum ratio from
            # ``shape_pi_capacitances`` — the segment-level cap matrix
            # captures the spatial asymmetry that drives gold's
            # ``Cs1 ≠ Cs2`` even though its absolute magnitudes are
            # quasi-static stubs (see TODO.md §5). The C binary
            # achieves the same effect through ``solve_node_equations``
            # at ``asitic_kernel.c`` (which reduces the segment Y to a
            # 2-port at the spiral ends).
            from reasitic.substrate import (
                shape_pi_capacitances,
                shape_shunt_admittance,
            )

            Y_total = shape_shunt_admittance(shape, tech, freq_ghz)
            try:
                C_p1, C_p2, _ = shape_pi_capacitances(
                    shape, tech, n_div=n_div,
                )
