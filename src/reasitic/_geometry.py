...
class Polygon:
    ...
    ``winding_hint``: optional override for ``edges()``'s CW/CCW
    normalisation. ``None`` keeps the legacy "always normalise to CW"
    behaviour. ``"ccw"`` forces CCW orientation for the centerline
    extraction — needed by the mmsq m2 flip-cascade where the
    intended polygon winding is opposite the m3 winding (per gdb
    trace of cmd_inductance_compute @ 0x0804ce78 showing C's m2
    spirals winding CCW vs m3's CW).
    """
    ...
    width: float = 0.0
    thickness: float = 0.0
    winding_hint: str | None = None

    def edges(self) -> list[Segment]:
        """Return the conductor-axis segments for this ribbon.

        Two storage conventions coexist in the codebase:

        * **Centerline polylines** (spirals, wires, capacitor plates,
          rings after the centerline rewrite): the ``vertices`` list
          *is* the conductor centerline; each consecutive pair forms a
          ribbon segment with ``self.width`` as the orthogonal width.

        * **Filled chamfered ribbons** (transformer / balun / symsq /
          sympoly / mmsquare polygons emitted through
          ``layout_polygons``): the ``vertices`` list is a 4-corner
          (or 5-vertex, first==last) outline of one ribbon segment.

        For the filled case we collapse the outline back to a single
        centerline segment: the two shortest edges are the ribbon's
        "ends", and their midpoints are the centerline endpoints. This
        keeps analysis (inductance, DC resistance, shunt cap)
        consistent with the C binary's one-segment-per-ribbon model
        (asitic_repl.c:0x0804ee74).
        """
        n = len(self.vertices)
        if n < 2:
            return []
        ...
        if is_closed:
            # 4-corner ribbon. Anchor the centerline direction to the
            # first long edge under a consistent polygon orientation
            # (always normalised to clockwise). This was the previous
            # behaviour — verified necessary for symsq / sympoly /
            # trans where the flip-cascade emits CCW polygons whose
            # mirrored coil L would otherwise diverge by 25 %.
            # mmsq remains slightly over (16 %) because its m2 layer
            # has a different outer-trace length than the C's m2.
            corners = self.vertices[: n - 1]
            ...
            # Default: normalise to CW (shoelace < 0). The
            # ``winding_hint`` field overrides this for shapes whose
            # flip cascade intends CCW (e.g. mmsq m2 layer).
            target_ccw = (self.winding_hint == "ccw")
            currently_ccw = shoelace > 0
            if target_ccw != currently_ccw:
                corners = list(reversed(corners))
            edge_lens = [
                corners[i].distance_to(corners[(i + 1) % n_corners])
                for i in range(n_corners)
            ]
            long_idx = max(range(n_corners), key=lambda i: edge_lens[i])
            before = (long_idx - 1) % n_corners
            after = (long_idx + 1) % n_corners
            a_start = corners[before]
            b_start = corners[long_idx]
            a_end = corners[after]
            b_end = corners[(long_idx + 2) % n_corners]
            m1 = Point(
                (a_start.x + b_start.x) * 0.5,
                (a_start.y + b_start.y) * 0.5,
                (a_start.z + b_start.z) * 0.5,
            )
            m2 = Point(
                (a_end.x + b_end.x) * 0.5,
                (a_end.y + b_end.y) * 0.5,
            ...
            return [
                Segment(
                    a=m1, b=m2, width=self.width,
                    thickness=self.thickness, metal=self.metal,
                )
            ]
        segs: list[Segment] = []
        for i in range(n - 1):
...
def _polygon_swap_edges_3d(polys: list[Polygon]) -> list[Polygon]:
    """Reverse each ribbon's centerline direction.

    Mirrors ``polygon_swap_edges_3d`` (decomp ``0x0807a2d4``) which
    swaps three vertex pairs inside each polygon record (centerline
    ends + top-edge ends + bottom-edge ends). In Python's 4-corner
    representation [v0, v1, v2, v3] this is equivalent to the
    permutation ``[v1, v0, v3, v2]`` — the polygon shape is unchanged
    but the centerline derived by :meth:`Polygon.edges` flips
    direction (its two "short end" midpoints swap).

    ASITIC calls this on every polygon in ``cmd_showldiv_format``
    after ``cmd_flip_apply`` while building each lower metal layer
    of an mmsquare stack — that flip alone reverses the linked-list
    order, but the per-polygon centerline-direction reversal is what
    actually makes the m2 spiral wind opposite to m3 so the
    cross-layer mutual inductance has the correct sign.
    """
    out: list[Polygon] = []
    for p in polys:
        n = len(p.vertices)
        if n < 5:  # not a closed 4-corner ribbon
            out.append(p)
            continue
        v0, v1, v2, v3 = p.vertices[0], p.vertices[1], p.vertices[2], p.vertices[3]
        # New corner order: [v1, v0, v3, v2, v1] (closed) — reverses
        # the polygon winding. Tag with winding_hint="ccw" so
        # Polygon.edges() does NOT renormalize back to CW (which
        # would undo the swap). The hint causes edges() to keep
        # the swapped order, giving a centerline that runs
        # opposite to the pre-swap direction.
        new_verts = [v1, v0, v3, v2, v1]
        out.append(Polygon(
            vertices=new_verts, metal=p.metal,
            width=p.width, thickness=p.thickness,
            winding_hint="ccw",
        ))
    return out
...
def _mmsquare_layout_polygons(shape: Shape, tech: Tech) -> list[Polygon]:
    ...
    # The flip-cascade between consecutive metal layers mirrors
    # ``cmd_mmsquare_build_geometry`` (asitic_repl.c:0x0805af5c):
    #
    #   frac = turns - round(turns)
    #   if frac >= 0 or frac == 0.5:   # condition for fliph
    #       pcVar10 = cmd_fliph_apply
    #       alternating = True
    #   else:                           # frac < 0 (and != 0.5)
    #       pcVar10 = cmd_flipv_apply
    #
    # Earlier this code had the conditional inverted — it used
    # cmd_flipv_apply for integer N when the C actually uses
    # cmd_fliph_apply (decomp lines 5652-5662). The integer-N
    # branch is the canonical N=2 mmsq case, so the wrong flip
    # axis was driving the gold m2 polygon mismatch (gold m2 inner
    # polygons shifted in Y, not X, vs Python's flipv).
    #
    # In ASITIC's convention ``cmd_flipv_apply`` mirrors X around
    # the bbox vertical centreline (left-right flip) and
    # ``cmd_fliph_apply`` mirrors Y around the horizontal centreline
    # (top-bottom flip). The integer-N mmsq stack is therefore a
    # top-bottom flip per layer.
    frac = shape.turns - round(shape.turns)
    if frac >= 0.0 or abs(frac - 0.5) < 1e-9:
        flip = _polygon_fliph_apply
        alternating = True
    else:
        flip = _polygon_flipv_apply
        alternating = False

    prev_polys = top_polys
    use_h_next = False
    for layer_idx in range(top_metal - 1, bot_metal - 1, -1):
        flipped = flip(prev_polys)
        flipped = list(reversed(flipped))
        # Note: ASITIC's cmd_mmsquare_build_geometry follows cmd_flip_apply
        # with cmd_showldiv_format → polygon_swap_edges_3d on every
        # polygon. Empirically the m2 centerline derived by
        # Polygon.edges() already comes out CCW after flip+reverse
        # alone (current at each azimuth is parallel to m3's), so
        # adding _polygon_swap_edges_3d here would over-correct
        # (drives total L → 0.04 nH instead of the ~2.99 nH gold for
        # mmsq L140 N=2). The remaining mmsq cross-layer error
        # (~12 %) appears to be a polygon-length discrepancy, not
        # a winding-direction one.
        flipped = _polygons_relayer(flipped, tech, layer_idx)
        out.extend(flipped)
        prev_polys = flipped
        if alternating:
            flip = _polygon_fliph_apply if use_h_next else _polygon_flipv_apply
            use_h_next = not use_h_next
...
def layout_polygons(shape: Shape, tech: Tech) -> list[Polygon]:
    ...
    if shape.kind == "capacitor":
        # Each plate is stored as a 2-vertex centerline; reconstruct
        # the rectangle from the centerline + width.
        out: list[Polygon] = []
        for p in shape.polygons:
            if len(p.vertices) < 2:
                continue
...
def square_spiral(
    ...
    * the very last segment is trimmed by ``1.5 × W`` to leave clearance
    ...
    # Resolve exit_metal. Per ``shape_args_LSWN`` (asitic_repl.c:0x08082110)
    # the C parses missing ``EXIT=`` as ``g_num_metal_layers - 2`` — the
    # second-from-top layer, so the spiral's inner end drops a via cluster
    # to a lower routing layer by default. Passing ``exit_metal=None``
    # explicitly disables the access routing (used by the MMSQ base
    # path, where the C first sets ``shape+0x6c = -1``).
    if isinstance(exit_metal, str):
        exit_idx: int | None = _resolve_metal(tech, exit_metal).index
    elif isinstance(exit_metal, int):
        exit_idx = exit_metal
    elif include_access and exit_metal is None:
        # Default: one layer below the spiral's metal (= num_metals - 2
        # when the spiral is on the top layer).
        exit_idx = metal_idx - 1 if metal_idx - 1 >= 0 else None
    else:
        exit_idx = None
    ...
    # Emit the exit cluster (via grid + lower-metal pad + lower-metal
    # exit lead) as additional polygons on the shape, so analysis
    # commands (MetArea, Res, Ind) see the via R / pad metal area /
    # lead R contribution. The C builder emits these via
    # ``lookup_via_for_metal_pair`` + ``display_list_append`` when
    # ``shape + 0x6c >= 0`` (cmd_square_build_geometry @
    # asitic_repl.c:0x08056670 line 3580+).
    if include_access and exit_idx is not None and exit_idx != metal_idx:
        access = _square_centerline_access_polygons(sh, tech)
        sh.polygons = polygons + access
    return sh


def _square_centerline_access_polygons(
    shape: Shape, tech: Tech
) -> list[Polygon]:
    """Emit via / exit-pad / exit-lead polygons for a centerline-stored
    square spiral.

    Mirrors the exit-routing branch of ``cmd_square_build_geometry``
    (asitic_repl.c:0x08056670, lines 3560+) which runs when
    ``shape + 0x6c >= 0``. The C reads the *last* polygon's endpoint
    geometry from offsets +0x18 / +0x20 / +0x44 / +0x5c (the 4
    corners of the terminal ribbon segment) to choose the via /
    pad placement; here we derive that placement from the
    centerline's last segment direction instead, because the
    Python builder stores the spiral as a polyline.
    """
    verts = shape.polygons[0].vertices if shape.polygons else []
    if len(verts) < 2 or shape.exit_metal is None:
        return []
    if shape.exit_metal == shape.metal:
        return []
    if shape.exit_metal < 0 or shape.exit_metal >= len(tech.metals):
        return []
    a = verts[-2]
    b = verts[-1]
    dx = b.x - a.x
    ...
    W = shape.width
    half = W * 0.5
    # Direction of current exiting the spiral (unit vector b → a).
    ux = -dx / L
    uy = -dy / L
    # Classify the last side to match the C's ``uVar19`` cases
    # (asitic_repl.c:0x08056670 lines 3560+):
    #   0 → top side, exit goes +x      (dx>0)
    #   1 → right side, exit goes -y    (dy<0)
    #   2 → bottom side, exit goes -x   (dx<0)
    #   3 → left side, exit goes +y     (dy>0)
    if abs(dx) > abs(dy):
        last_side = 0 if dx > 0 else 2
    else:
        last_side = 3 if dy > 0 else 1
    # Build a synthetic 4-corner "ribbon" for the last segment so we
    # can reuse ``_square_access_polygons`` (which is written against
    # the C ribbon-polygon storage).
    nx = -dy / L * half
    ny = dx / L * half
    corners = [
        (a.x + nx, a.y + ny),
    ...
    last_poly = Polygon(
        vertices=[
            Point(corners[0][0], corners[0][1], z),
            Point(corners[1][0], corners[1][1], z),
            Point(corners[2][0], corners[2][1], z),
            Point(corners[3][0], corners[3][1], z),
            Point(corners[0][0], corners[0][1], z),
        ],
        metal=shape.metal,
        width=W,
        thickness=shape.polygons[0].thickness,
    )
    _ = ux, uy  # direction info encoded in last_side
    return _square_access_polygons(shape, tech, last_poly, last_side)
...
def ring(
    ...
    *,
    ...
    Mirrors ASITIC's ``cmd_ring_build_geometry`` (decomp
    ``0x0805b450``): emit ``sides − 1`` polygons at a *constant*
    radius, leaving a single ``gap`` (in degrees) at the start
    phase. Each polygon is a trapezoid with outer vertices at
    ``radius`` and inner vertices at ``radius − W/cos(π/sides)``.

    The earlier implementation routed through :func:`polygon_spiral`
    with turns=1, which inadvertently applied a radial decrement
    per side and shrank the effective ring length by ~6 % on the
    canonical R60 / W8 / sides=32 case (Python L = 0.218 nH vs
    gold 0.230 nH). Constant radius matches the C exactly.
    """
    if sides < 3:
        raise ValueError("ring needs at least 3 sides")
    metal_rec = _resolve_metal(tech, metal)
    metal_idx = metal_rec.index
    z = metal_rec.d + metal_rec.t * 0.5
    thickness = metal_rec.t

    gap_rad = math.radians(abs(gap))
    cos_half = math.cos(math.pi / sides)
    radial_w = width / cos_half
    inner_r = radius - radial_w
    # Per-side angular step skips the gap arc (C: lVar8).
    per_side = (2.0 * math.pi - gap_rad) / (sides - 1)
    start_angle = phase + gap_rad * 0.5

    # Outer / inner vertices at constant radius. ``sides`` vertices
    # define ``sides − 1`` polygons (the gap eats the wrap-around).
    angles = [start_angle + k * per_side for k in range(sides)]
    outer = [(radius * math.cos(a), radius * math.sin(a)) for a in angles]
    inner = [(inner_r * math.cos(a), inner_r * math.sin(a)) for a in angles]
    xs = [v[0] for v in outer + inner]
    ys = [v[1] for v in outer + inner]
    dx = x_origin + (max(xs) - min(xs)) * 0.5 - (max(xs) + min(xs)) * 0.5
    dy = y_origin + (max(ys) - min(ys)) * 0.5 - (max(ys) + min(ys)) * 0.5
    polys: list[Polygon] = []
    for i in range(sides - 1):
...
def symmetric_square(
    ...
    *,
    ...
    # Eagerly materialise the CIF-equivalent ribbon polygons. The C
    # binary's ``cmd_symsq_build_geometry`` populates the linked list
    # at ``shape + 0xa8`` up-front (see asitic_repl.c:0x08059854), so
    # analysis commands can walk ``shape.polygons`` directly.
    sh.polygons = _symsq_layout_polygons(sh, tech)
    return sh
...
def capacitor(
    ...
    *,
    ...
    # Each plate is stored as a single centerline segment (length × width)
    # so analyses like ``cmd_metalarea_print`` (length × width) and DC
    # resistance (rsh · length / width) match the C binary, which keeps
    # one polygon record per plate with a centerline endpoint pair plus
    # the orthogonal width attribute. The 4-corner CIF/GDS rectangle is
    # reconstructed in :func:`layout_polygons` for export.
    def plate(metal_idx: int, z: float, thickness: float) -> Polygon:
        return Polygon(
            vertices=[
                Point(x_origin, y_origin + width * 0.5, z),
                Point(x_origin + length, y_origin + width * 0.5, z),
            ],
            metal=metal_idx,
            width=width,
            thickness=thickness,
    ...
    polygons = [
        plate(top.index, z_top, top.t),
        plate(bot.index, z_bot, bot.t),
    ]
    return Shape(
        name=name,
        polygons=polygons,
...
def symmetric_polygon(
    ...
    *,
    ...
    # Materialise CIF-equivalent ribbon polygons up-front so analysis
    # commands have segment data to walk (mirrors cmd_sympoly_build_geometry
    # at asitic_repl.c:0x0805a45c which populates ``shape + 0xa8``).
    sh.polygons = _sympoly_layout_polygons(sh, tech)
    return sh
...
def multi_metal_square(
    ...
    * ``metals=[m3, m2]`` — backward-compatible explicit list.
    ...
    base = square_spiral(
        ...
        x_origin=x_origin, y_origin=y_origin,
        # MMSQ's C builder sets ``shape + 0x6c = -1`` before calling
        # cmd_square_build_geometry (asitic_repl.c:0x0805af5c) so the
        # square base has no exit cluster; the via cluster between
        # adjacent metal layers is emitted by
        # _mmsquare_layout_polygons instead.
    ...
    # The C ``cmd_mmsquare_build_geometry`` (decomp ``0x0805af5c``)
    # populates the linked list at ``shape + 0xa8`` with ribbons for
    # every metal layer in the stack, not just the top one. We mirror
    # that by replacing the single-metal base polygons with the full
    # multi-metal layout — :func:`metal_area`, DC resistance and the
    # inductance matrix all walk ``shape.polygons``.
    sh.polygons = _mmsquare_layout_polygons(sh, tech)
    return sh
...
def balun(
    ...
    *,
    ...
    # BALUN turn-count split: ``cmd_balun_edit_args`` (decomp
    # ``0x08085e84``) sets
    #   primary_N   = round((N + 1) / 2)
    #   secondary_N = primary_N - 1
    # The user's N is the **total** turn count distributed across the
    # two coils. The secondary's coil is one turn shorter than the
    # primary. The current _balun_secondary_layout_polygons builder is
    # parameterised on the *user* N (it assumes ``which`` distinguishes
    # the half-coils), so we hand it ``turns`` unchanged here. The
    # secondary's reduced size emerges from the alternating-ring pattern
    # in that builder.
    base_shape = Shape(
        name=name,
        polygons=[],
    ...
    # Eagerly materialise the CIF-equivalent ribbon polygons so analysis
    # routines (metal_area, DC resistance, self-inductance) can walk
    # ``shape.polygons`` directly. Mirrors the symsq / transformer path
    # — the C binary's ``cmd_balun_build_geometry`` populates the linked
    # list at ``shape + 0xa8`` up-front, so callers don't need a separate
    # "render" step.
    if which == "primary":
        base_shape.polygons = _balun_primary_layout_polygons(base_shape, tech)
    else:
        base_shape.polygons = _balun_secondary_layout_polygons(base_shape, tech)
    return base_shape
...
def _balun_primary_layout_polygons(shape: Shape, tech: Tech) -> list[Polygon]:
    ...
    # Right-side ILEN bridge stub: connects the centre-U's right arm
    # (Y=L/2+ILEN..L+ILEN/2) to the outermost bottom-U's right arm
    # (Y=ILEN/2..L/2). Two stacked W×(ILEN/2) rectangles at X=L-W..L.
    # Only needed when ``even_ks == [0]`` (= N=2 case) — for N≥3 the
    # outer-to-inner slant fills this gap instead.
    # Verified against gold ``balun_..._L180_W8_S3_N2_..._primary.cif``.
    if even_ks and max(even_ks) == 0 and ILEN > 0:
        bridge_x0 = X + L - W
        bridge_x1 = X + L
        bridge_mid = Y + L * 0.5 + ILEN * 0.5
        for y0, y1 in [
            (Y + L * 0.5, bridge_mid), (bridge_mid, Y + L * 0.5 + ILEN),
        ]:
            polys.append(_polygon_record_to_poly(
                [(bridge_x0, y0), (bridge_x0, y1), (bridge_x1, y1), (bridge_x1, y0)],
                metal_rec, W,
            ))
    ...
    # Via cluster + M2 chamfered trace bridging the outermost and
    # innermost rings — emitted ONLY when there's an inner ring to
    # bridge to. For ``N == 2`` (``even_ks == [0]``, ``k_inner == 0``)
    # the primary is just the outer ring with no inner ring to
    # connect, so the cluster + M2 trace must be skipped. Earlier
    # versions of this builder emitted them unconditionally, which
    # inflated R_DC by 3× on the L180/N=2 case (gold 1.665 Ω vs
    # 4.85 Ω). Verified against gold CIFs:
    # - ``balun_..._L180_W8_S3_N2_..._primary.cif`` — m3-only
    # - ``balun_..._L200_W8_S3_N3_..._primary.cif`` — m3 + LM2 + LVIA3
    exit_idx = shape.exit_metal
    k_inner = max(even_ks) if even_ks else 0
    if (exit_idx is not None and exit_idx != shape.metal
            and even_ks and k_inner > 0):
        via_rec, via_idx = None, -1
        for i, v in enumerate(tech.vias):
            if {v.top, v.bottom} == {shape.metal, exit_idx}:
                via_rec, via_idx = v, i
...
def _balun_secondary_layout_polygons(shape: Shape, tech: Tech) -> list[Polygon]:
    ...
    metal_rec = tech.metals[shape.metal]
    n_int = round(shape.turns)
    # BALUN's C ``cmd_balun_edit_args`` (asitic_repl.c:0x08085e84)
    # splits the user N as ``primary_N = round((N+1)/2)``,
    # ``secondary_N = primary_N - 1``. The secondary's ring count
    # then follows the alternating-rings pattern of the
    # cmd_symsq_build_geometry path called against the secondary
    # shape — empirically this collapses to **zero** rings when
    # ``secondary_N <= 1`` (the gold balun_..._N2 secondaries are
    # just centre-tap stubs), and to one ring at ``k=1`` for the
    # ``secondary_N >= 1`` cases that fit (gold balun_..._N3
    # secondaries are an inner ring + centre stubs). Encode that
    # observation here rather than ``[k for k in range(n_int)…]``
    # which over-emitted rings for N=2.
    primary_n = round((n_int + 1.0) * 0.5)
    secondary_n = max(0, primary_n - 1)
    if n_int <= 2:
        odd_ks: list[int] = []
    else:
        odd_ks = [k for k in range(1, n_int, 2)][:secondary_n]
    polys: list[Polygon] = []
    ...
    # LEFT centre-tap stubs: always emitted for the secondary, even
    # when there are no rings (gold balun_..._N2 secondary has only
    # the stubs). Located at offset ``k_stub * pitch`` from the
    # x-origin where ``k_stub`` matches the outermost odd ring when
    # any rings are emitted, else 1 (= same x-offset as the would-be
    # k=1 ring, which is where the primary's ring complements it).
    if ILEN > 0:
        k_stub = min(odd_ks) if odd_ks else 1
        stub_x0 = X + k_stub * pitch
        stub_x1 = stub_x0 + W
        stub_mid = Y + L * 0.5 + ILEN * 0.5
        for y0, y1 in [
            (Y + L * 0.5, stub_mid), (stub_mid, Y + L * 0.5 + ILEN),
