...
def _segment_pair_mutual(a: Segment, b: Segment) -> float:
    ...
    if pair is not None:
        ...
        if sep < 1e-12:
            ...
            return 0.0
        # Replace the centerline separation ``sep`` with the bar-pair
        # GMD from ``wire_inductance_far_field_kernel`` (decomp
        # asitic_kernel.c:0x08063ca0). Split the offset into the
        # W-direction (lateral xy gap) and T-direction (z gap)
        # components so the kernel's 16-corner Maxwell integral
        # gets the right cross-section geometry — important for
        # cross-layer (z-stacked) bars whose offset is in T not W.
        from reasitic.inductance.decomp_ports import (
            wire_inductance_far_field_kernel,
        )
        other_axes = [i for i in range(3) if i != ax_a]
        d_other = _coord(a.a, other_axes[0]) - _coord(b.a, other_axes[0])
        d_z = _coord(a.a, other_axes[1]) - _coord(b.a, other_axes[1])
        if a.width > 0 and b.width > 0 and a.thickness > 0 and b.thickness > 0:
            sep_gmd = wire_inductance_far_field_kernel(
                a.width, b.width, a.thickness, b.thickness,
                abs(d_other), abs(d_z),
            )
            if sep_gmd > 0:
                sep = sep_gmd
...
def compute_self_inductance(shape: Shape) -> float:
    ...
    Wide-stacked-plate shapes (cap / mmsq) are imperfect under this
    centerline-only filament treatment because the centerline-to-
    centerline mutual between wide rectangles over-counts the
    spatially-integrated cross-plate coupling. For those shapes, use
    :func:`reasitic.inductance.filament.solve_inductance_mna` with
    skin-depth-aware subdivision instead — it matches the binary's
    LRMAT output within a few percent.
    """
    segs = shape.segments()
    if not segs:
        return 0.0
