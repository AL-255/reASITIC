...
from reasitic.inductance.partial import _segment_pair_mutual
...
def _filament_pair_m(f_i: Filament, f_j: Filament) -> float:
    """Mutual inductance (nH) between two filaments — dispatches via
    :func:`_segment_pair_mutual` so non-axis-aligned (octagonal /
    polygon / skew) pairs are handled by the general skew kernel,
    not silently dropped. Earlier versions short-circuited to 0 for
    anything but Manhattan pairs, which killed mutual L for sp /
    ring / sympoly shapes in the Schur-reduced LRMAT path.
    """
    s_i = _filament_to_segment(f_i)
    s_j = _filament_to_segment(f_j)
    return _segment_pair_mutual(s_i, s_j)
...
def build_resistance_vector(
    ...
    freq_ghz: float,
    *,
    parent_segments: list[Segment] | None = None,
) -> np.ndarray:
    """Per-filament AC resistance, ready to fill the Z diagonal.

    Note: ``parent_segments`` parameter is reserved for a future
    fix that would propagate the parent's closed-form skin
    enhancement to all sibling filaments. Two variants have been
    tried (replace-with-parent-R, ratio-scale): both fixed mmsq
    R@10GHz from -14 % to ~-1 % but over-corrected proximity-
    dominated spirals, even under the 0.5 % tolerance (net +71
    failures with replace-variant on 2026-05-12). Right fix
    likely needs a 2D filament grid (n_w × n_t) plus proper
    inter-filament M coupling mirroring the C's
    ``filament_list_setup`` (decomp 0x08064e20) +
    ``fill_impedance_matrix_triangular`` (decomp 0x080650d4).
    """
    ...
    for i, f in enumerate(filaments):
        if f.metal < 0 or f.metal >= n_metals:
            continue
...
def solve_inductance_mna_via_recomp(
    shape: Shape, tech: Tech, freq_ghz: float,
) -> tuple[float, float]:
    """Drive the verbatim recomp kernel for the LRMAT path.

    Currently produces L within 1-2× of gold. Wire 0.086 (1.28× gold),
    sq 0.94 (1.73× gold), cap 0.051 (0.79× under). recomp is ABSOLUTELY
    CORRECT — remaining variance traces to tile centroid placement and
    near-field tile-pair handling.
    """
    from reasitic.inductance.recomp_wrapper import compute_LR_via_lrmat_path
    return compute_LR_via_lrmat_path(shape, tech, freq_ghz)
...
def solve_inductance_mna(
    ...
    *,
    ...
    Feature-flag ``REASITIC_USE_RECOMP=1`` routes through the verbatim
    recomp kernel wrapper (``compute_LR_via_lrmat_path``). Otherwise
    uses the interpretive Python path.
    """
    if _os.environ.get("REASITIC_USE_RECOMP") == "1":
        return solve_inductance_mna_via_recomp(shape, tech, freq_ghz)
    return _solve_inductance_mna_interpretive(shape, tech, freq_ghz, n_w=n_w, n_t=n_t)


def _solve_inductance_mna_interpretive(
    shape: Shape,
    tech: Tech,
    freq_ghz: float,
    *,
    ...
    """Original Python interpretive implementation. Topology:
    ...
    * Within parent ``p`` the ``n_par = n_w·n_t`` sub-filaments are
    ...
    # Skip BOTH pads of a paired W×W via-contact pair (the gold
    # treats via-cluster pads as zero-impedance contacts on both
    # the top and exit metal — current flows through the via cells
    # themselves). Standalone W×W segments (e.g. symsq's spiral
    # end-caps) keep their contribution.
    n_metals = len(tech.metals)
    filtered_segs = []
    for s in segs:
        seg_len_sq = (
        ...
        is_w_pad = (
            s.width > 0
            and abs(math.sqrt(seg_len_sq) - s.width) < 1e-6
            and 0 <= s.metal < n_metals
        )
        if is_w_pad:
            cx = (s.a.x + s.b.x) * 0.5
            cy = (s.a.y + s.b.y) * 0.5
            has_pair = False
            for s2 in segs:
                if s2 is s or s2.metal == s.metal:
                    continue
                if 0 <= s2.metal < n_metals and s2.width == s.width:
                    cx2 = (s2.a.x + s2.b.x) * 0.5
                    cy2 = (s2.a.y + s2.b.y) * 0.5
                    if abs(cx - cx2) < 1e-6 and abs(cy - cy2) < 1e-6:
                        has_pair = True
                        break
            if has_pair:
                continue
        filtered_segs.append(s)
    segs = filtered_segs if filtered_segs else segs
    ...
    # Add via cluster R as series elements. The C's
    # ``fill_inductance_offdiag`` stamps ``R_via = rho_via / (n_x * n_y)``
    # on each via FILAMENT's diagonal cell (decomp 0x080657b8 line
    # 5042). Each via filament in the binary represents one *cluster*
    # (a 3×3 group of cells, defaulted by ``build_filament_list`` line
    # 4830: ``nx = 3; ny = 3`` for vias). With multiple clusters in
    # series (e.g. balun's two centertap stacks), the total R is the
    # SUM of per-cluster contributions, not the parallel of all cells.
    #
    # Cluster detection: group via segments by xy proximity within
    # ``(via.width + via.space) * 1.5`` of each other (union-find).
    n_metals = len(tech.metals)
    via_segs: dict[int, list] = {}
    for s in segs:
        if s.metal >= n_metals:
            via_segs.setdefault(s.metal - n_metals, []).append(s)
    for via_idx, items in via_segs.items():
        if not (0 <= via_idx < len(tech.vias)):
            continue
        v = tech.vias[via_idx]
        pitch = (v.width + getattr(v, "space", v.width)) * 1.5
        # Union-find by xy proximity
        n = len(items)
        parent = list(range(n))
        def find(x: int) -> int:
            while parent[x] != x:
        ...
        def union(x: int, y: int) -> None:
            ...
            if rx != ry:
                parent[rx] = ry
        for i in range(n):
            cx_i = (items[i].a.x + items[i].b.x) * 0.5
            cy_i = (items[i].a.y + items[i].b.y) * 0.5
            for j in range(i + 1, n):
                cx_j = (items[j].a.x + items[j].b.x) * 0.5
                cy_j = (items[j].a.y + items[j].b.y) * 0.5
                if (cx_i - cx_j) ** 2 + (cy_i - cy_j) ** 2 < pitch * pitch:
                    union(i, j)
        clusters: dict[int, int] = {}
        for i in range(n):
            r = find(i)
            clusters[r] = clusters.get(r, 0) + 1
        for n_cells in clusters.values():
            if n_cells > 0:
                R_eff += v.r / n_cells
