...
def coupled_wire_self_inductance(
    ...
    # The Grover §24 formula. The Ghidra decomp shows the natural
    # logs as ``0.6931... * X`` because the ``fyl2x`` instructions
    # got dropped — the assembly at 0x0804cc06 loads ``ln(2)`` via
    # ``fldln2`` before each ``fyl2x``, which computes
    # ``ln(2)·log2(X) = ln(X)``. The decomp's ``0.6931 * X`` is
    # therefore actually ``ln(X)``.
    L_a = math.log((R4 + 1.0) / R3)
    L_b = math.log((b + R4) / R1)
    L_c = math.log((a + R4) / R2)
    L_60 = a * 60.0
    ...
    term1 = a3 * (math.log((b + R3) / a) - L_b) / (b * 24.0)
    term2 = a3 * (math.log((R1 + 1.0) / a) - L_a) / (b2 * 24.0)
    term3 = a * (R3 - R4) / 20.0
    term4 = (1.0 - R2) / (b2 * 60.0 * a)
    term5 = a * (R1 - R4) / (b2 * 20.0)
    term6 = (math.log(a + R1) - L_c) / (b2 * 24.0)
    term7 = L_c * 0.25
    term8 = (a * L_b) / (4.0 * b)
    term9 = a * L_a * 0.25
    term10 = (R2 - R4) / (a * 20.0)
    term11 = (b2 * (b - R2)) / L_60
    term12 = (b2 * (math.log((a + R3) / b) - L_c)) / 24.0
    term13 = (math.log(b + R2) - L_b) / ((a * b) * 24.0)
    term14 = (math.log((R2 + 1.0) / b) - L_a) * b2 / (a * 24.0)
