...
class Repl:
    """Interactive REPL emulating the original ASITIC command set.

    Each ``cmd_*`` method implements one of the ASITIC commands
    documented in ``run/doc/`` of the upstream reverse-engineering
    project.  Typical usage::

        from reasitic.cli import Repl
        from reasitic import parse_tech_file

        repl = Repl(tech=parse_tech_file("BiCMOS.tek"))
    ...
    def cmd_square(self, args: dict[str, str]) -> None:
        """SQ: square spiral. ``EXIT=<lower-layer>`` routes the inner
        exit through that layer's centre stub + via cluster (the
        C's ``cmd_square_build_geometry`` 3D-bridge path)."""
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        sh = square_spiral(
            args["NAME"],
    ...
    def cmd_cap(self, name: str, freq_ghz: float = 1.0) -> None:
        """CAP <name> [<freq_ghz>] (case 415).

        Mirrors the verbose branch of ``analyze_capacitance_driver``
        (asitic_kernel.c:0x08052c50) — at a given frequency the C
        binary emits::
        ...
        where ``Total Cap = -1 / (ω · Im(Z_sum)) · 1e15`` and
        ``Total Resistance = Re(Z_sum)``, with
        ``Z_sum = 1 / Σ_ij Y_cap_ij`` over the full *segment-level*
        cap matrix (i.e. before any L-matrix node reduction). Our
        Python segment-cap pipeline (:func:`analyze_capacitance_driver`
        in :mod:`reasitic.substrate.segment_cap`) builds the same
        matrix structure, but the underlying Green's-function values
        are quasi-static stubs — the magnitudes won't match the
        binary until the full Sommerfeld FFT integration is ported.
        """
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        sh = self.shapes[name]
        from reasitic.substrate import shape_shunt_admittance
        Y_sum = shape_shunt_admittance(sh, self.tech, freq_ghz)
        omega = 2.0 * math.pi * freq_ghz * 1.0e9
        # C-faithful formula (asitic_kernel.c:0x08052c50):
        #     Cap = -1 / (ω · Im(Z_sum)) · 1e15
        #     R   = Re(Z_sum)
        # with ``Z_sum = 1 / Y_sum``. Until the §7 substrate-loss
        # topology fix landed, ``Y_sum`` returned the Pi-reduced
        # parallel-RC admittance — low-Q, and the C's formula
        # over-amplified the cap by ``(1 + 1/Q²)``. The series-Yue
        # topology in ``shape_shunt_admittance`` now puts Q ≫ 1
        # (oxide-cap in series with substrate-loss G), so the C
        # formula gives accurate cap and R values for canonical
        # BiCMOS spirals within ~10 % of gold at 1 GHz.
        if abs(Y_sum) <= 1.0e-15 or omega <= 0:
            cap_ff = 0.0
            r_ohm = float("inf")
        else:
            Z_sum = 1.0 / Y_sum
            r_ohm = float(Z_sum.real)
            imZ = float(Z_sum.imag)
            cap_ff = (-1.0 / (omega * imZ)) * 1.0e15 if imZ < 0 else 0.0
        print(f"At {freq_ghz:.3f} GHz:")
        print(f"Total Capacitance = {cap_ff:.3f} (fF)")
        print(f"Total Resistance = {r_ohm:.3f}")
    ...
    def cmd_trans(self, args: dict[str, str]) -> None:
        ...
        Accepts ``NAME`` or ``PNAME`` (the C binary's ``TRANS``
        syntax uses ``PNAME`` for the primary shape name) and
        ``SNAME`` for the secondary. ``METAL2`` defaults to ``EXIT``
        if absent. The C's ``TRANS`` materialises BOTH coils;
        mirror that here so ``CIFSAVE``/analysis can address each
        by name.
        """
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        pname = args.get("PNAME", args.get("NAME"))
        if pname is None:
            raise KeyError("TRANS requires NAME or PNAME")
        sname = args.get("SNAME")
        metal2 = args.get("METAL2", args.get("EXIT"))
        common = dict(
            length=float(args["LEN"]),
            width=float(args["W"]),
            spacing=float(args["S"]),
            ...
            metal_secondary=metal2,
            x_origin=float(args.get("XORG", "0")),
            y_origin=float(args.get("YORG", "0")),
        )
        primary = transformer(pname, which="primary", **common)
        self.shapes[primary.name] = primary
        if sname:
            secondary = transformer(sname, which="secondary", **common)
            self.shapes[secondary.name] = secondary

    def cmd_balun(self, args: dict[str, str]) -> None:
        """BALUN: stacked counter-wound spirals.

        Mirrors the C binary's ``BALUN`` command which builds both
        the primary and secondary coils, named ``<NAME>-P`` and
        ``<NAME>-S`` respectively. Accepts ``W`` (centre width) or
        ``W1`` (the C's syntax).
        """
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        width_arg = args.get("W", args.get("W1"))
        if width_arg is None:
            raise KeyError("BALUN requires W or W1")
        base_name = args["NAME"]
        kwargs = dict(
            length=float(args["LEN"]),
            width=float(width_arg),
            spacing=float(args["S"]),
            turns=float(args["N"]),
            tech=self.tech,
            metal=args.get("METAL", 0),
        ...
        primary = balun(f"{base_name}-P", which="primary", **kwargs)
        secondary = balun(f"{base_name}-S", which="secondary", **kwargs)
        self.shapes[primary.name] = primary
        self.shapes[secondary.name] = secondary
    ...
    def cmd_symsq(self, args: dict[str, str]) -> None:
        ...
        Like ``SYMPOLY``, the C accepts ``METAL=primary:METAL2=exit``
        and ``ILEN`` for the centre-tap span. Both were silently
        dropped by the Python wrapper before, which is why the
        alternating slants landed on the primary layer and the
        centre-tap span fell back to ``W+S``.
        """
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        sh = symmetric_square(
    ...
    def cmd_sympoly(self, args: dict[str, str]) -> None:
        ...
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        # The C's SYMPOLY accepts ``METAL=<primary>:METAL2=<exit>`` —
        # the ``METAL2`` field selects the via-cluster slant metal
        # for the alternating-ring transitions. When omitted the
        # slants stay on the primary layer.
    ...
    def cmd_mmsquare(self, args: dict[str, str]) -> None:
        ...
        Accepts the C binary's ``METAL=<top>:EXIT=<bottom>`` form
        (mirroring ``MMSQ NAME=...:METAL=m3:EXIT=m2``) as well as
        the legacy ``METALS=m3,m2`` comma-separated list. When
        ``METAL`` + ``EXIT`` are both present, every layer from
        top down to bottom (inclusive) is used — matching the C's
        ``cmd_mmsquare_build_geometry``
        (``asitic_repl.c:0x0805af5c``) traversal.
        """
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        if "METALS" in args:
            metals_list: list[int | str] = [
                m.strip() for m in args["METALS"].split(",")
            ]
            kwargs = dict(metals=metals_list)
        else:
            kwargs = dict(
                metal=args.get("METAL", 0),
                exit_metal=args.get("EXIT"),
    ...
    def cmd_lrmat(
        ...
        """LRMAT (case 0x213) — emit the partial-inductance matrix
        and the AC-Z lumped summary at the given frequency.

        Mirrors ``cmd_lmat_print`` (asitic_repl.c:0x08066214). The C
        signature is ``(shape_a, shape_b, freq_GHz, filename)``; it
        solves the per-segment Z matrix at ``freq_GHz`` via
        ``solve_inductance_matrix``, sums every entry, and emits

            L = Im(Σ Z_ij) / ω        R = Re(Σ Z_ij)

        followed by the matrix dump (to file when ``filename`` is
        given, else stdout).
        """
        if self.tech is None:
            raise RuntimeError("no tech file loaded")
        ...
        if freq_ghz is not None and freq_ghz > 0:
            # Build Σ Z_ij at this frequency. ``Σ L_ij`` is the C's
            # lumped self-inductance (= ``compute_self_inductance`` in
            # the loss-less limit); ``Σ R_ij`` is the AC resistance
            # from skin-effect ``compute_ac_resistance``. The C also
            # carries the full off-diagonal Z, but for analysis
            # purposes the lumped sums are equivalent.
            L_nH = compute_self_inductance(sh)
            try:
                from reasitic.resistance import compute_ac_resistance
                R_ac = compute_ac_resistance(sh, self.tech, freq_ghz)
            except Exception:
                R_ac = 0.0
            L_H = L_nH * 1.0e-9
            print(f"L = {L_H:10.4g}\t\tR = {R_ac:10.4f}")
    ...
    def _execute_inner(self, line: str) -> bool:
        ...
        if head_upper in ("LOAD-TECH", "T", "TECH"):
            ...
            parts = rest.split()
            if not parts:
                print("Usage: CAP <name> [<freq_ghz>]")
                return True
            freq = float(parts[1]) if len(parts) > 1 else 1.0
            self.cmd_cap(parts[0], freq)
            ...
            parts = rest.split()
            if not parts:
                print("Usage: LRMAT <name> [<freq_ghz>] [<output_path>]")
                return True
            freq: float | None = None
            path = None
            for tok in parts[1:]:
                # Try float first (the C's third arg is freq_GHz, e.g.
                # ``LRMAT name 1.0``); fall back to a filename for the
                # 4th-arg ``filename`` case.
                try:
                    freq = float(tok)
                except ValueError:
                    path = tok
            self.cmd_lrmat(parts[0], freq, path)
